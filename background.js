...
var requestTimeout = 1000 * 60;  // 2 seconds
var rotation = 0;
...
function getvaluUrl() {
  return "https://valu.is/";
}

function getMyPageUrl() {
  // "zx" is a valu query parameter that is expected to contain a random
  // string and may be ignored/stripped.
  console.log("getvalname:"+ getValuName());
  return getvaluUrl() + getValuName();
}

function getValuName(){
  if(!localStorage.hasOwnProperty("valu_name")){
    return "miyamaru"
  }else{
    return localStorage.valu_name;
  }
}

function isvaluUrl(url) {
  // Return whether the URL starts with the valu prefix.
  return url.startsWith(getvaluUrl());
}

// A "loading" animation displayed while we wait for the first response from
// valu. This animates the badge text with a dot that cycles from left to
// right.
...
function updateIcon() {
  if (!localStorage.hasOwnProperty('unreadCount')) {
    chrome.browserAction.setIcon({path:"valu_not_logged_in.png"});
    chrome.browserAction.setBadgeBackgroundColor({color:[190, 190, 190, 230]});
    chrome.browserAction.setBadgeText({text:"?"});
  } else {
    chrome.browserAction.setIcon({path: "valu_logged_in.png"});
  ...
  try {
    xhr.onreadystatechange = function() {
      ...
      if (xhr.responseXML) {
        ...
        var fullCountSet = xmlDoc.evaluate('//li[@class="watch"]/b',
            xmlDoc, valuNSResolver, XPathResult.ANY_TYPE, null);
        var fullCountNode = fullCountSet.iterateNext();
        if (fullCountNode) {
          handleSuccess(fullCountNode.textContent);
          return;
        } else {
          console.error(chrome.i18n.getMessage("valucheck_node_error"));
        }
      }
    ...
    xhr.open("GET", getMyPageUrl(), true);
    xhr.responseType = "document";
    xhr.send(null);
  } catch(e) {
    console.error(chrome.i18n.getMessage("valucheck_exception", e));
    handleError();
  }
}

function valuNSResolver(prefix) {
  if(prefix == 'valu') {
...
function goToInbox() {
  ...
  chrome.tabs.getAllInWindow(undefined, function(tabs) {
    for (var i = 0, tab; tab = tabs[i]; i++) {
      if (tab.url && isvaluUrl(tab.url)) {
        console.log('Found valu tab: ' + tab.url + '. ' +
    ...
    console.log('Could not find valu tab. Creating one...');
    chrome.tabs.create({url: getvaluUrl()});
  });
}
...
var filters = {
  ...
  url: [{urlContains: getvaluUrl().replace(/^https?\:\/\//, '')}]
};

function onNavigate(details) {
  if (details.url && isvaluUrl(details.url)) {
    console.log('Recognized valu navigation to: ' + details.url + '.' +
                'Refreshing count...');
    startRequest({scheduleRequest:false, showLoadingAnimation:false});
  }
}
